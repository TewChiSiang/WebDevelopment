...
const Attendance = ({ auth }) => {
  ...
  const generateQRData = () => {
    ...
    return JSON.stringify({
      ...
      courseTime: courseDetails.course_start_time,
      ...
      <Container className="p-2">
          ...
          <Card.Body className="tw-p-8">
            <Form>
                ...
                >
                  ...
                  {courses.map((course) => (
                    <option key={course.id} value={course.id}>
                      {course.course_id} - {course.course_name} - {course.weekday} - {course.course_start_time} - {course.course_end_time}
                    </option>
                  ))}
                </Form.Select>
              </Form.Group>
                  ...
                  <div className="tw-bg-gray-100 tw-p-6 tw-rounded-lg tw-inline-block">
                    {getSelectedCourseDetails() && (
                      <div className="tw-mb-4">
                        ...
                        <p className="tw-text-gray-600 tw-mb-2">{getSelectedCourseDetails().weekday}</p>
                        <p className="tw-text-gray-600 tw-mb-2">{getSelectedCourseDetails().course_start_time} - {getSelectedCourseDetails().course_end_time}</p>
                        <div className="tw-flex tw-justify-center tw-items-center tw-mr-5 tw-gap-2 tw-mb-2">
              ...
              <Modal
                ...
                centered
                size="lg"
                className="tw-fixed tw-inset-0 tw-flex tw-items-center tw-justify-center"
              >
                {/* 背景遮罩 */}
                <div
                ...
                {/* Modal 内容 */}
                <div
                  className="tw-relative tw-z-50 tw-p-8 tw-bg-white tw-rounded-xl tw-shadow-2xl tw-overflow-y-auto"
                >
                  <button
                    onClick={() => setShowQRModal(false)}
                    className="tw-absolute tw-right-1 tw-top-1 tw-p-0 tw-bg-transparent tw-border-none"
                  >
        ...
        {selectedCourse && (
          <div className="tw-mt-8">
            <StudentList students={getCurrentCourseStudents()} courseId={selectedCourse} />
          </div>
        )}
      </Container>
    </div>
